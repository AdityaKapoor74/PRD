...
class Policy(nn.Module):
	def __init__(self, obs_input_dim, num_agents, num_actions, attention_type, device):
		...
		self.attention_value = nn.Sequential(
			nn.Linear(obs_output_dim, 256, bias=True), 
			nn.Tanh(),
			nn.Linear(256, 256, bias=True), 
			nn.Tanh(),
			)
		self.layer_norm = nn.LayerNorm(obs_output_dim)
		...
		# ********************************************************************************************************
		final_input_dim = obs_output_dim + curr_agent_output_dim
	...
	def reset_parameters(self):
		...
		nn.init.orthogonal_(self.final_policy_layers[0].weight, gain=gain)
		nn.init.orthogonal_(self.final_policy_layers[2].weight, gain=gain)
		nn.init.orthogonal_(self.final_policy_layers[4].weight)
	...
	def forward(self, states):
		...
		attention_values = self.attention_value(states_key_embed)
		node_features = torch.matmul(weight, attention_values)
		node_features = self.layer_norm(states_key_embed+node_features)
		...
		return self.final_policy_layers(curr_agent_node_features)
'''
