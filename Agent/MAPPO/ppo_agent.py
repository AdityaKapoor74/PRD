...
class PPOAgent:
	...
	def __init__(
		...
		if self.env_name == "paired_by_sharing_goals":
			...
			obs_dim = 2*3 + 5
			self.critic_network = TransformerCritic_threshold_pred(obs_dim, 128, obs_dim+self.num_actions, 128, 128, 1, self.num_agents, self.num_actions, self.num_heads_critic, self.device).to(self.device)
