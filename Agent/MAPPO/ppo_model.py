...
class MLPPolicyNetwork(nn.Module):
	def __init__(self,state_dim,num_agents,action_dim):
		...
		self.actor = nn.Sequential(
	        nn.Linear(state_dim*num_agents,128),
	        nn.LeakyReLU(),
	        nn.Linear(128, 128),
	        nn.LeakyReLU(),
	...
	def reset_parameters(self):
		...
		nn.init.xavier_uniform_(self.actor[0].weight, gain=gain)
		nn.init.xavier_uniform_(self.actor[2].weight, gain=gain)
		nn.init.xavier_uniform_(self.actor[4].weight, gain=gain)
...
class GATCritic(nn.Module):
	...
	def reset_parameters(self):
		...
		nn.init.xavier_uniform_(self.critic[0].weight, gain=gain_leaky)
		nn.init.xavier_uniform_(self.critic[2].weight, gain=gain_leaky)
	...
	def forward(self, states, policies, actions):
		...
		Value = self.critic(node_features)
