...
class PPOAgent:
	...
	def get_q_v_values(self, state_allies, state_enemies, one_hot_actions, rnn_hidden_state_q, rnn_hidden_state_v, indiv_dones, episode):
		with torch.no_grad():
	...
	def update_HAPPO(self, episode):
		...
		for agent_id in agent_permutation:
			...
			with torch.no_grad():
				...
				probs_old = Categorical(dists_old_.squeeze(-2))
			...
			for _ in range(self.n_epochs):
				...
				hidden_state_actor = hidden_state_actor.reshape(self.rnn_num_layers_actor, -1, self.num_agents, self.rnn_hidden_actor)			
			...
			self.buffer.factor = self.buffer.factor*torch.exp((logprobs_new.reshape(*target_shape).cpu()-logprobs_old_.reshape(*target_shape).cpu())*masks_.reshape(*target_shape)).reshape(self.update_ppo_agent*data_chunks, self.data_chunk_length)
