...
class MAA2C:
	...
	def update(self,trajectory,episode):
		...
		shared_rewards = torch.FloatTensor([sars[9] for sars in trajectory]).to(self.device)

		self.agents.update(states_critic,next_states_critic,one_hot_actions,one_hot_next_actions,actions,states_actor,next_states_actor,rewards,dones, episode, shared_rewards)
	...
	def run(self):  
		...
		for episode in range(1,self.max_episodes+1):
			...
			for step in range(1, self.max_time_steps+1):
				...
				# every agent gets individual reward
				...
				if self.learn:
					if all(dones) or step == self.max_time_steps:
						...
						trajectory.append([states_critic,next_states_critic,one_hot_actions,one_hot_next_actions,actions,states_actor,next_states_actor,indiv_rewards,dones, shared_rewards])
						print("*"*100)
