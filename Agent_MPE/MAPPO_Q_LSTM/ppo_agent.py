...
class PPOAgent:
	...
	def update(self,episode):
		...
		hidden_state_pol = torch.stack(self.buffer.h_out_pol, dim=0).to(self.device)
		cell_state_pol = torch.stack(self.buffer.cell_out_pol, dim=0).to(self.device)
		hidden_state_critic = torch.stack(self.buffer.h_out_critic, dim=0).to(self.device).permute(1,0,2,3)
		cell_state_critic = torch.stack(self.buffer.cell_out_critic, dim=0).to(self.device)
