...
class LICAAgent:
	...
	def __init__(
		...
		# Q Network
		self.actor = RNNAgent(self.height, self.width, self.channel, self.rnn_hidden_dim, self.num_agents, self.num_actions, self.device).to(self.device)
		
		self.critic = LICACritic(self.height, self.width, self.channel, self.mixing_embed_dim, self.num_actions, self.num_agents, self.num_hypernet_layers).to(self.device)
		self.target_critic = LICACritic(self.height, self.width, self.channel, self.mixing_embed_dim, self.num_actions, self.num_agents, self.num_hypernet_layers).to(self.device)
	...
	def get_action(self, state, last_one_hot_action):
		with torch.no_grad():
			...
			dists = self.actor(state.to(self.device), last_one_hot_action.to(self.device))
	...
	def update(self, buffer, episode):
		...
		state_batch = torch.FloatTensor(np.array(buffer.states)).permute(0, 1, 2, 5, 3, 4)
		...
		for _ in range(self.num_updates):
			...
			for t in range(mask_batch.shape[1]):
				...
				states_slice = state_batch[:,t].reshape(-1, self.channel, self.height, self.width)
				last_one_hot_action_slice = last_one_hot_actions_batch[:, t].reshape(-1, self.num_actions)
				
				dist = self.actor(states_slice.to(self.device), last_one_hot_action_slice.to(self.device))
			...
			mix_loss = self.critic(probs.to(self.device), state_batch.reshape(-1, self.channel, self.height, self.width).to(self.device)).squeeze(-1).squeeze(-1).reshape(state_batch.shape[0], -1)
			...
			Qs = self.critic(one_hot_actions_batch.to(self.device), state_batch.reshape(-1, self.channel, self.height, self.width).to(self.device)).squeeze(-1).squeeze(-1).reshape(state_batch.shape[0], -1) * mask_batch.to(self.device)
			
			target_Qs = self.target_critic(one_hot_actions_batch.to(self.device), state_batch.reshape(-1, self.channel, self.height, self.width).to(self.device)).squeeze(-1).squeeze(-1).reshape(state_batch.shape[0], -1)
