...
class PPOAgent:
	...
	def __init__(
		...
		self.critic_network = TransformerCritic(
			height=self.height, width=self.width, in_channel=self.channel,
			obs_act_input_dim=self.height*self.width*self.channel+self.num_actions, 
		...
		self.target_policy_network = Policy(
			height=self.height, width=self.width, in_channel=self.channel,
			num_agents=self.num_agents, 
			num_actions=self.num_actions, 
			device=self.device).to(self.device)
	...
	def get_actions(self,state):
		with torch.no_grad():
			...
			dists = self.target_policy_network.forward(state).squeeze(0)
