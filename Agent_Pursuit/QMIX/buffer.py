...
class ReplayMemory:
	def __init__(self, capacity, max_episode_len, num_agents, height, width, channel, action_shape):
		...
		self.buffer = dict()
		self.buffer['state'] = np.zeros((capacity, self.max_episode_len, num_agents, height, width, channel), dtype=np.float32)
		self.buffer['next_state'] = np.zeros((capacity, self.max_episode_len, num_agents, height, width, channel), dtype=np.float32)
	...
	def end_episode(self):
		...
		self.t = 0
		# clear previous data
		self.buffer['state'][self.episode] = np.zeros((self.max_episode_len, self.num_agents, self.height, self.width, self.channel), dtype=np.float32)
		...
		self.buffer['next_state'][self.episode] = np.zeros((self.max_episode_len, self.num_agents, self.height, self.width, self.channel), dtype=np.float32)
