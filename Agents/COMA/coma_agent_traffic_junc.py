...
from coma_model_traffic_junc import *
...
class COMAAgent:
	...
	def __init__(
		...
		obs_input_dim = 4
		self.critic_network = TransformerCritic(obs_input_dim, 128, obs_input_dim+self.num_actions, 128, 128+128, self.num_actions, self.num_agents, self.num_actions, self.device).to(self.device)
		self.target_critic_network = TransformerCritic(obs_input_dim, 128, obs_input_dim+self.num_actions, 128, 128+128, self.num_actions, self.num_agents, self.num_actions, self.device).to(self.device)
	...
	def update(self,states_critic,next_states_critic,one_hot_actions,one_hot_next_actions,actions,states_actor,next_states_actor,rewards,dones,episode):		
		...
		'''
		probs = self.policy_network(states_actor)

		Q_values, weights_value = self.critic_network(states_critic, one_hot_actions)
		...
		target_Q_values, target_weights_value = self.target_critic_network(states_critic, one_hot_actions)
