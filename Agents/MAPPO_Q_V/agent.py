...
class PPOAgent:
	...
	def update(self, episode):
		...
		for _ in range(self.n_epochs):
			...
			advantage, masking_rewards, mean_min_weight_value = self.calculate_advantages_based_on_exp(Value, Values_old, rewards.to(self.device), dones.to(self.device), torch.mean(weights_prd_old.detach(), dim=1), masks.to(self.device), episode)
