...
class PPOAgent:
	...
	def V_update(self, episode):
		...
		V_values_old, weights_value_old = self.critic_network_old(old_states, old_agent_global_positions, agent_ids, old_probs.squeeze(-2), old_one_hot_actions)
		...
		for _ in range(self.n_epochs):
			...
			Value, weights_value = self.critic_network(old_states, old_agent_global_positions, agent_ids, old_probs.squeeze(-2), old_one_hot_actions)
			Value = Value.reshape(-1,self.num_agents,self.num_agents)
