...
# 	def __init__(self, in_channels, obs_input_dim, num_agents, num_actions, value_normalization, device):
...
# 		return Value, Q_value, weights, scores
...
class Q_network(nn.Module):
	...
	def forward(self, states, policies, actions):
		...
		if 'semi-hard' in self.attention_type:
			...
			scores = (torch.matmul(query_obs,key_obs.transpose(2,3))/math.sqrt(self.d_k)) + hard_score.unsqueeze(-2)
			weight = F.softmax(scores ,dim=-1)
			weights = self.weight_assignment(weight.squeeze(-2))
		else:
			scores = torch.matmul(query_obs,key_obs.transpose(2,3))/math.sqrt(self.d_k)
