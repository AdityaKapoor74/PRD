...
from a2c_v4 import PolicyNetwork, CriticNetwork
...
class A2CAgent:
	...
	def __init__(
		...
		self.weight_input_dim = 2*3 #2*3+2 # (pose,vel,goal pose, paired agent goal pose) --> observations
		self.weight_output_dim = 1
		self.obs_z_input_dim = self.weight_input_dim + self.num_actions
		self.obs_z_output_dim = 16
		self.final_input_dim = 2*self.weight_input_dim+self.num_actions #self.obs_z_output_dim + self.weight_input_dim
