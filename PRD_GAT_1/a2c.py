...
class SoftAttentionInput(nn.Module):
	...
	def reduce_func(self, nodes):
		# reduce UDF for equation (3) & (4)
		# equation (3)
		# alpha = torch.sigmoid(torch.exp(nodes.mailbox['score'] / math.sqrt(self.d_k)).clamp(-5, 5))
...
class SoftAttentionWeight(nn.Module):
	...
	def reduce_func(self, nodes):
		# reduce UDF for equation (3)
		# equation (3)
		# w = torch.sigmoid(torch.exp(nodes.mailbox['score'] / math.sqrt(self.d_k)).clamp(-5, 5))
...
class CriticNetwork(nn.Module):
	...
	def forward(self, g, policies, actions):
		...
		obs_final, weights = self.weight_layer(g,g.ndata['pairings'],policies,actions)
