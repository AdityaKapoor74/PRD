...
class ScalarDotProductCriticNetwork_Q(nn.Module):
	def __init__(self, obs_input_dim, obs_output_dim, obs_act_input_dim, obs_act_output_dim, final_input_dim, final_output_dim, num_agents, num_actions, threshold=0.1):
		super(ScalarDotProductCriticNetwork_Q, self).__init__()
		...
		self.final_qvalue_layer_1 = nn.Linear(final_input_dim, 64, bias=False)
		self.final_qvalue_layer_2 = nn.Linear(64, final_output_dim, bias=False)
		# ********************************************************************************************************	
	...
	def reset_parameters(self):
		...
		nn.init.xavier_uniform_(self.final_qvalue_layer_1.weight, gain=gain)
		nn.init.xavier_uniform_(self.final_qvalue_layer_2.weight, gain=gain)
	...
	def forward(self, states, policies, actions):
		...
		Value = F.leaky_relu(self.final_qvalue_layer_1(node_features))
		Value = self.final_qvalue_layer_2(Value)

		return Value, ret_weight



class ScalarDotProductCriticNetwork_V(nn.Module):
	def __init__(self, obs_input_dim, obs_output_dim, obs_act_input_dim, obs_act_output_dim, final_input_dim, final_output_dim, num_agents, num_actions, threshold=0.1):
		super(ScalarDotProductCriticNetwork_V, self).__init__()
