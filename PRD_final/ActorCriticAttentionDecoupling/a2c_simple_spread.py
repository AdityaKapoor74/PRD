...
class ScalarDotProductCriticNetwork(nn.Module):
	def __init__(self, obs_act_input_dim, obs_act_output_dim, goal_input_dim, goal_output_dim, final_input_dim, final_output_dim, num_agents, num_actions, threshold=0.1):
		...
		self.query_layer = nn.Linear(goal_input_dim, goal_output_dim, bias=False)
	...
	def forward(self, states, policies, actions, goal_states):
		...
		# QUERIES
		query_goals = self.query_layer(goal_states)

		# SCORE
		# we do not perform a transpose here because we need softmax over the score that landmark i to agent j --> Vij (Value estimate for landmark i not conditioned on agent j's actions)
		score_obs_actions = torch.bmm(query_goals,key_obs_actions.transpose(1,2)).reshape(-1,1)
		...
		current_node_states = goal_states.unsqueeze(-2).repeat(1,1,self.num_agents,1)
...
class DualAttentionCriticNetwork(nn.Module):
	def __init__(self, obs_act_input_dim, obs_act_output_dim, goal_input_dim, goal_output_dim, final_input_dim, final_output_dim, num_agents, num_actions, threshold=0.1):
		...
		# OBSERVATIONS
		self.key_layer_obs = nn.Linear(goal_input_dim, goal_output_dim, bias=False)

		self.query_layer_obs = nn.Linear(goal_input_dim, goal_output_dim, bias=False)

		self.attention_value_layer_obs = nn.Linear(goal_input_dim, goal_output_dim, bias=False)
		# dimesion of key
		self.d_k_obs = goal_output_dim
		...
		self.query_layer_obs_act = nn.Linear(goal_input_dim, goal_output_dim, bias=False)
	...
	def forward(self, states, policies, actions, goal_states):
		...
		# QUERIES
		query_goals = self.query_layer_obs_act(goal_states)
		...
		# KEYS
		key_obs = self.key_layer_obs(goal_states)
		# QUERIES
		query_obs = self.query_layer_obs(goal_states)
		# ATTENTION VALUES
		attention_values_obs = self.attention_value_layer_obs(goal_states).unsqueeze(1).repeat(1,1,self.num_agents,1).reshape(goal_states.shape[0],self.num_agents,self.num_agents,-1)
		# SCORE
