...
from a2c_agent_simple_spread import A2CAgent
import datetime
...
class MAA2C:
	...
	def __init__(self,env, gif=True, save=True):
		...
		self.weight_obs_act_dictionary = {}

		for i in range(self.num_agents):
			landmark = 'landmark %d' % i
			self.weight_obs_act_dictionary[landmark] = {}
			for j in range(self.num_agents):
				agent_name = 'agent %d' % j
				self.weight_obs_act_dictionary[landmark][agent_name] = 0


		self.weight_obs_dictionary = {}

		for i in range(self.num_agents):
			landmark = 'landmark %d' % i
			self.weight_obs_dictionary[landmark] = {}
			for j in range(self.num_agents):
				landmark_ = 'landmark %d' % j
				self.weight_obs_dictionary[landmark][landmark_] = 0
		...
		for i in range(self.num_agents):
			landmark = 'landmark %d' % i
			self.distance_dictionary[landmark] = {}
			for j in range(self.num_agents):
				agent_name = 'agent %d' % j
				self.distance_dictionary[landmark][agent_name] = 0
	...
	def calculate_indiv_weights_obs_act(self,weights):
		...
		for i in range(self.num_agents):
			...
			for j in range(self.num_agents):
				agent = 'agent %d' % j
				self.weight_obs_act_dictionary[landmark][agent] = weights_per_agent[i][j].item()

	def calculate_indiv_weights_obs(self, weights):
		weights_per_landmark = torch.sum(weights,dim=0) / weights.shape[0]

		for i in range(self.num_agents):
			landmark = 'landmark %d' % i
			for j in range(self.num_agents):
				landmark_ = 'landmark %d' % j
				self.weight_obs_dictionary[landmark][landmark_] = weights_per_landmark[i][j].item()
	...
	def update(self,trajectory,episode):
		...
		if not(self.gif) and self.save:
			...
			# # Single Attention
			# self.calculate_indiv_weights_obs_act(weights_obs_act)
			# for i in range(self.num_agents):
			# 	landmark = 'landmark %d' % i
			# 	self.writer.add_scalars('Weights_Landmark_Agent/Average_Weights/'+landmark,self.weight_obs_act_dictionary[landmark],episode)
			...
			for i in range(self.num_agents):
				landmark = 'landmark %d' % i
				self.writer.add_scalars('Weights/Average_Weights_Landmark_Agent/'+landmark,self.weight_obs_act_dictionary[landmark],episode)
			...
			self.writer.add_scalar('Weights/Entropy_Obs_Act', entropy_weights.item(), episode)
			# ENTROPY OF WEIGHTS_OBS
			entropy_weights = -torch.mean(torch.sum(weights_obs * torch.log(torch.clamp(weights_obs, 1e-10,1.0)), dim=2))
			self.writer.add_scalar('Weights/Entropy_Obs', entropy_weights.item(), episode)
	...
	def split_states(self,states):
		...
		for i in range(self.num_agents):
			...
			goal_states.append(states[i][2])

		states_critic = np.asarray(states_critic)
		states_actor = np.asarray(states_actor)
		goal_states = np.asarray(goal_states)

		return states_critic, states_actor, goal_states
	...
	def run(self,max_episode,max_steps):  
		for episode in range(1,max_episode):
			...
			states_critic,states_actor, goal_states = self.split_states(states)
