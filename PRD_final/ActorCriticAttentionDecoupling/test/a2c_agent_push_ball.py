...
class A2CAgent:
	...
	def __init__(
		...
		# MLP POLICY
		self.policy_network = MLPPolicyNetworkPushBall(2*3, self.num_agents, self.num_actions).to(self.device)
	...
	def update(self,states_agent,next_states_agent,one_hot_actions,one_hot_next_actions,actions,states_ball,next_states_ball,rewards,dones):
		probs = self.policy_network.forward(states_agent, states_ball)

		V_values, weights_agent_ball, weights_agent_agent = self.critic_network.forward(states_agent, probs.detach(), one_hot_actions)
		...
		# V values
		return value_loss,policy_loss,entropy,grad_norm_value,grad_norm_policy,weights_agent_ball,weights_agent_agent