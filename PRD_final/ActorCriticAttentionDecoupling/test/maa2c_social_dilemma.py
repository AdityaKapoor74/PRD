...
from a2c_agent_social_dilemma import A2CAgent
import datetime
...
class MAA2C:
	...
	def __init__(self, env, dictionary):
		...
		self.weight_agent_agent_dictionary = {}
		for i in range(self.num_agents):
			agent_name = 'agent %d' % i
			self.weight_agent_agent_dictionary[agent_name] = {}
			for j in range(self.num_agents):
				agent_name_ = 'agent %d' % j
				self.weight_agent_agent_dictionary[agent_name][agent_name_] = 0

		self.weight_agent_goal_dictionary = {}
		for i in range(self.num_agents):
			agent_name = 'agent %d' % i
			self.weight_agent_goal_dictionary[agent_name] = {}
			for j in range(self.num_agents):
				goal_name = 'goal %d' % j
				self.weight_agent_goal_dictionary[agent_name][goal_name] = 0
	...
	def get_actions(self,states_agent, states_goal):
		actions = self.agents.get_action(states_agent, states_goal)
		return actions
	...
	def calculate_indiv_weights_agent_agent(self,weights):
		...
		for i in range(self.num_agents):
			...
			for j in range(self.num_agents):
				agent_name_ = 'agent %d' % j
				self.weight_agent_agent_dictionary[agent_name][agent_name_] = weights_per_agent[i][j].item()

	def calculate_indiv_weights_agent_goal(self,weights):
		weights_per_agent = torch.sum(weights,dim=0) / weights.shape[0]
		...
		for i in range(self.num_agents):
			...
			for j in range(self.num_agents):
				goal_name = 'goal %d' % j
				self.weight_agent_goal_dictionary[agent_name][goal_name] = weights_per_agent[i][j].item()
	...
	def update(self,trajectory,episode):
		...
		states_goal = torch.FloatTensor([sars[5] for sars in trajectory]).to(self.device)
		next_states_goal = torch.FloatTensor([sars[6] for sars in trajectory]).to(self.device)
		...
		value_loss,policy_loss,entropy,grad_norm_value,grad_norm_policy,weights_agent_goal,weights_agent_agent = self.agents.update(states_agent,next_states_agent,one_hot_actions,one_hot_next_actions,actions,states_goal,next_states_goal,rewards,dones)
		...
		if self.save:
			...
			self.calculate_indiv_weights_agent_agent(weights_agent_agent)
			for i in range(self.num_agents):
				agent_name = 'agent %d' % i
			...
			self.writer.add_scalar('Weights_Critic/Entropy_Agent_Goal', entropy_weights.item(), episode)

			entropy_weights = -torch.mean(torch.sum(weights_agent_agent * torch.log(torch.clamp(weights_agent_agent, 1e-10,1.0)), dim=2))
			self.writer.add_scalar('Weights_Critic/Entropy_Agent_Agent', entropy_weights.item(), episode)
	...
	def run(self):  
		for episode in range(1,self.max_episodes+1):
			...
			states_agent,states_goal = self.split_states(states)
			...
			for step in range(1, self.max_time_steps+1):
				...
				if self.gif:
					...
					with torch.no_grad():
						actions = self.get_actions(states_agent, states_goal)
				else:
					actions = self.get_actions(states_agent, states_goal)
				...
				next_states_agent,next_states_goal = self.split_states(next_states)
				next_states_goal = next_states_goal[0]

				# next actions
				next_actions = self.get_actions(next_states_agent, next_states_goal)
				...
				if self.learn:
					if all(dones) or step == self.max_time_steps:
						...
						trajectory.append([states_agent,next_states_agent,one_hot_actions,one_hot_next_actions,actions,states_goal,next_states_goal,rewards,dones])
